...
public class ScytheItem extends Item {
    public ScytheItem(ToolMaterial toolMaterial, float attackDamage, float attackSpeed, Item.Settings settings) {
    ...
    private static ToolComponent createToolComponent() {
        ...
        return new ToolComponent(
                List.of(ToolComponent.Rule.ofAlwaysDropping(RegistryEntryList.of(cobwebEntry), 15.0F),
                        ToolComponent.Rule.of(RegistryEntryList.of(bambooEntry), 1.5F)), 1.0F, 1, true);
    }
    ...
    public float getMiningSpeed(ItemStack stack, BlockState state) {
        if (state.isIn(BlockTags.HOE_MINEABLE)) return 15.0F;
        if (state.isOf(Blocks.VINE) || state.isOf(Blocks.GLOW_LICHEN)) return 8.0F;
    ...
    public boolean canBeEnchantedWith(ItemStack stack, RegistryEntry<Enchantment> enchantment, EnchantingContext context) {
        ...
        boolean denied = enchantment.matchesKey(Enchantments.FORTUNE) || enchantment.matchesKey(Enchantments.IMPALING) || enchantment.matchesKey(Enchantments.SWEEPING_EDGE)
                || enchantment.matchesKey(Enchantments.KNOCKBACK) || enchantment.matchesKey(Enchantments.BREACH) || enchantment.matchesKey(Enchantments.DENSITY)
                || enchantment.matchesKey(Enchantments.WIND_BURST);
        if (denied) {
            return false;
        }
