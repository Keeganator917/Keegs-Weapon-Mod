...
public class ServerRef {
    ...
    public static boolean weaponForgeStructuresEnabled() {
        ...
        return server.overworld().getGameRules().get(ModGamerules.DO_WEAPON_FORGE_STRUCTURES_SPAWN);
    }

    public static boolean weaponForgeRecipeBlockingEnabled() {
        if (server == null) return true;
        return server.overworld().getGameRules().get(ModGamerules.DO_WEAPON_FORGE_RECIPE_BLOCKING);
    }
}
